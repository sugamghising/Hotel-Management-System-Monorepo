...
export class RoomsService {
  ...
  async create(
    ...
    const existing = await this.roomRepo.findByRoomNumber(
      hotelId,
      input.roomNumber,
    );
    if (existing) {
      throw new ConflictError(
        `Room number '${input.roomNumber}' already exists in this hotel`,
      );
    }
    ...
    const orgLimits = await this.checkOrgLimits(
      organizationId,
      currentRoomCount + 1,
    );
    if (!orgLimits.valid) {
      throw new BadRequestError(
        orgLimits.message ?? "Organization room limit exceeded",
      );
    }
  ...
  async findByHotel(
    ...
    const { rooms, total } = await this.roomRepo.findByHotel(
      hotelId,
      filters,
      pagination,
    );
    ...
    for (const row of gridData) {
      ...
      if (rooms) {
        rooms.push({
          ...
          ...(row.nextArrival
            ? {
                nextArrival: new Date(row.nextArrival)
                  .toISOString()
                  .split("T")[0],
              }
            : {}),
  ...
  async update(
    ...
    if (input.roomTypeId && input.roomTypeId !== room.roomTypeId) {
      ...
      if (hasReservations) {
        throw new BadRequestError(
          "Cannot change room type while room has active reservations",
        );
      }
    }
  ...
  async updateStatus(
    ...
    const lastCleanedAt = ["VACANT_CLEAN", "OCCUPIED_CLEAN"].includes(
      input.status,
    )
      ? new Date()
      : undefined;
    ...
    logger.info(
      `Room status changed: ${room.roomNumber} ${room.status} -> ${input.status}`,
      {
        roomId: id,
  ...
  async setOutOfOrder(
    ...
    const nextReservation = await this.roomRepo.getNextReservation(
      id,
      new Date(),
    );
    if (nextReservation) {
  ...
  async bulkUpdateStatus(
    ...
    const updatedCount = await this.roomRepo.bulkUpdateStatus(
      input.roomIds,
      input.status,
      hotelId,
    );
    ...
    logger.info(
      `Bulk room status update: ${updatedCount} rooms to ${input.status}`,
      {
        hotelId,
  ...
  async checkAvailability(
    ...
    return this.roomRepo.checkAvailability(
      roomId,
      checkIn,
      checkOut,
  ...
  async getHistory(
    ...
    return history.map((h: RoomStatusHistoryEntry) => {
      const changes = h.changes as Record<
        string,
        Record<string, unknown>
      > | null;
      const metadata = h.metadata as Record<string, unknown> | null;
      const previousStatus = changes?.["before"]?.["status"] as
        | RoomStatus
        | undefined;
      const newStatus = changes?.["after"]?.["status"] as
        | RoomStatus
        | undefined;
  ...
  private async checkOrgLimits(
    ...
    const rawMax =
      process.env["ORG_MAX_ROOMS"] ?? process.env["ORGANIZATION_MAX_ROOMS"];
    ...
    return {
      ...
      currentReservation: currentReservation
        ? {
            ...
            checkOut:
              (currentReservation.reservation["checkOutDate"] as Date) ??
              new Date(),
            nights: Math.ceil(
              ((
                (currentReservation.reservation["checkOutDate"] as Date) ??
      ...
      nextReservation: nextReservation
        ? {
            ...
            checkIn:
              (nextReservation.reservation["checkInDate"] as Date) ??
              new Date(),
